...
final case class JsPath(private [value] val positions: Vector[Position])
  ...
  def appended(i: Int): JsPath = JsPath(positions appended Index(requireNonNull(i)))

  /** Alias for `prepended` */
  @`inline` def \(index: Int): JsPath = prepended(requireNonNull(index))

  def prepended(i: Int): JsPath = JsPath(positions prepended Index(requireNonNull(i)))

  /** Alias for `appended` */
  @`inline` def /(key: String): JsPath = appended(requireNonNull(key))

  def appended(name: String): JsPath = JsPath(positions appended Key(requireNonNull(name)))

  /** Alias for `prepended` */
  @`inline` def \(key: String): JsPath = prepended(requireNonNull(key))

  def prepended(key: String): JsPath = JsPath(positions prepended Key(requireNonNull(key)))

  @`inline` def /(path: JsPath): JsPath = appended(requireNonNull(path))

  def appended(path: JsPath): JsPath = JsPath(positions ++ requireNonNull(path).positions)

  @`inline` def \(path: JsPath): JsPath = prepended(requireNonNull(path))

  def prepended(path: JsPath): JsPath = JsPath(requireNonNull(path).positions ++: positions)
...
object JsPath
  ...
  def toKey: Prism[Position, String] =
    {
  ...
  def toIndex: Prism[Position, Int] =
    {
      Prism((value: Position) => value match
      {
        case Index(i) => Some(i)
