...
import json.value.ValueParserFactory.ValueParser
                      ...
                      ) extends Parser[JsObj]
{
  ...
  private[json] val objDeserializer = ValueParserFactory.ofObjSpec(required,
                                                                   deserializers
                                                                   )
  ...
  {
    Try(value.dslJson.deserializeToJsObj(requireNonNull(bytes),
  ...
  def parse(str: String
    ...
    Try(value.dslJson.deserializeToJsObj(requireNonNull(str).getBytes,
                                         this.objDeserializer
                                         )
        ) match
    {
  ...
  def parse(inputStream: InputStream,
           ): Try[JsObj] = Try(value.dslJson.deserializeToJsObj(requireNonNull(inputStream),
                                                                this.objDeserializer
                                                                )
                               )
...
case class JsArrayParser(private[json] val deserializer: ValueParser) extends Parser[JsArray]
{
  ...
  def parse(bytes: Array[Byte]): Either[InvalidJson, JsArray] =
    Try(value.dslJson.deserializeToJsArray(requireNonNull(bytes),
                                           this.deserializer
                                           )
        ) match
    {
  ...
  def parse(str: String): Either[InvalidJson, JsArray] =
    Try(value.dslJson.deserializeToJsArray(requireNonNull(str).getBytes(),
                                           this.deserializer
                                           )
        ) match
    {
   ...
   */
  def parse(inputStream: InputStream
           ): Try[JsArray] = Try(value.dslJson.deserializeToJsArray(requireNonNull(inputStream),
                                                                    this.deserializer
                                                                    )
                                 )
...
object JsArrayParser
{
  ...
  def parse(inputStream: InputStream): Try[JsArray] =
  {
    ...
    try
    {
      parser = value.jacksonFactory.createParser(requireNonNull(inputStream))
  ...
  def parse(bytes: Array[Byte]): Either[InvalidJson, JsArray] =
  {
    ...
    try
    {
      parser = value.jacksonFactory.createParser(requireNonNull(bytes))
      val event: JsonToken = parser.nextToken
      if (event eq START_OBJECT)
  ...
  def parse(str: String): Either[InvalidJson, JsArray] =
  {
    ...
    try
    {
      parser = value.jacksonFactory.createParser(requireNonNull(str))
      val event: JsonToken = parser.nextToken
      if (event eq START_OBJECT)
        Left(InvalidJson.jsArrayExpected)
      else
  ...
  private[json] def createDeserializers(spec: Seq[JsSpec],
...
{
  private[json] val fn: String => java.util.function.Function[com.dslplatform.json.JsonReader[_], json.value.JsValue] = key =>
...
object JsObjParser
{
  ...
  private[json] def createDeserializers(spec: Map[SpecKey, JsSpec],
