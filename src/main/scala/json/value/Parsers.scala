...
import json.value.InternalError._
import json.value.spec.{Invalid, Result}
...
private[json] object Parsers
{
                              ...
                              ): ValParser =
  {
    ...
    then (reader: R) => arrOfIntParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                       p(_)
                                                                       )
    else if nullable && !elemNullable
    then (reader: R) => arrOfIntParser.nullOrArrayEachSuchThat(reader,
                                                               p(_)
                                                               )
    else if !nullable && elemNullable
    then (reader: R) => arrOfIntParser.arrayWithNullEachSuchThat(reader,
                                                                 p(_)
                                                                 )
    else (reader: R) => arrOfIntParser.arrayEachSuchThat(reader,
                                                         p(_)
                                                         )
  }
  ...
  /**
   *
   * @param typeCondition     condition to check if the json.value has the expected type
   * @param converter         function to convert the json.value to the expected type
   * @param spec              the specification that the json.value has to conform
   * @param errorTypeSupplier if the json.value doesn't have the expected type,
   ...
   * @param errorSpecSupplier function that returns the error throw if the json.value doesnt expect the given spec
   * @tparam R the type of the json.value (primitive type or JsObj or JsArray or Json)
   * @return a function to test that a json.value has the expected type and conforms a given spec
   */
  ...
  {
    if nullable && elemNullable
    then (reader: R) => arrOfLongParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                        p(_)
                                                                        )
    else if nullable && !elemNullable
    then (reader: R) => arrOfLongParser.nullOrArrayEachSuchThat(reader,
                                                                p(_)
                                                                )
    else if !nullable && elemNullable
    then (reader: R) => arrOfLongParser.arrayWithNullEachSuchThat(reader,
                                                                  p(_)
                                                                  )
    else (reader: R) => arrOfLongParser.arrayEachSuchThat(reader,
                                                          p(_)
                                                          )
  }
  ...
  {
    if nullable && elemNullable
    then (reader: R) => arrOfDecimalParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                           p(_)
                                                                           )
    else if nullable && !elemNullable
    then (reader: R) => arrOfDecimalParser.nullOrArrayEachSuchThat(reader,
                                                                   p(_)
                                                                   )
    else if !nullable && elemNullable
    then (reader: R) => arrOfDecimalParser.arrayWithNullEachSuchThat(reader,
                                                                     p(_)
                                                                     )
    else (reader: R) => arrOfDecimalParser.arrayEachSuchThat(reader,
                                                             p(_)
                                                             )
  }
  ...
  {
    if nullable && elemNullable
    then (reader: R) => arrOfIntegralParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                            p(_)
                                                                            )
    else if nullable && !elemNullable
    then (reader: R) => arrOfIntegralParser.nullOrArrayEachSuchThat(reader,
                                                                    p(_)
                                                                    )
    else if !nullable && elemNullable
    then (reader: R) => arrOfIntegralParser.arrayWithNullEachSuchThat(reader,
                                                                      p(_)
                                                                      )
    else (reader: R) => arrOfIntegralParser.arrayEachSuchThat(reader,
                                                              p(_)
                                                              )
  }
  ...
  {
    if nullable && elemNullable
    then (reader: R) => arrOfNumberParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                          p(_)
                                                                          )
    else if nullable && !elemNullable
    then (reader: R) => arrOfNumberParser.nullOrArrayEachSuchThat(reader,
                                                                  p(_)
                                                                  )
    else if !nullable && elemNullable
    then (reader: R) => arrOfNumberParser.arrayWithNullEachSuchThat(reader,
                                                                    p(_)
                                                                    )
    else (reader: R) => arrOfNumberParser.arrayEachSuchThat(reader,
                                                            p(_)
                                                            )
  }
  ...
  def ofArrayOfStrEachSuchThat(p: String => Result,
    ...
    if nullable && elemNullable
    then (reader: R) => arrOfStrParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                       p(_)
                                                                       )
    else if nullable && !elemNullable
    then (reader: R) => arrOfStrParser.nullOrArrayEachSuchThat(reader,
                                                               p(_)
                                                               )
    else if !nullable && elemNullable
    then (reader: R) => arrOfStrParser.arrayWithNullEachSuchThat(reader,
                                                                 p(_)
                                                                 )
    else (reader: R) => arrOfStrParser.arrayEachSuchThat(reader,
                                                         p(_)
                                                         )
  ...
  {
    if nullable && elemNullable
    then (reader: R) => arrOfValParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                       p(_)
                                                                       )
    else if nullable && !elemNullable
    then (reader: R) => arrOfValParser.nullOrArrayEachSuchThat(reader,
                                                               p(_)
                                                               )
    else if !nullable && elemNullable
    then (reader: R) => arrOfValParser.arrayWithNullEachSuchThat(reader,
                                                                 p(_)
                                                                 )
    else (reader: R) => arrOfValParser.arrayEachSuchThat(reader,
                                                         p(_)
                                                         )
  }
               ...
               ): ValParser =
  {
    (reader: R) =>
    {
      ...
      {
        val deserializer = JsObjSpecWithRequiredKeysDeserializer(required,
                                                                 keyDeserializers
                                                                 )
        if nullable
        then deserializer.nullOrValue(reader)
  ...
  {
    val deserializer = JsArrayOfObjSpecDeserializer(if required.isEmpty then JsObjSpecDeserializer(keyDeserializers) else JsObjSpecWithRequiredKeysDeserializer(required, keyDeserializers))
    if nullable && elemNullable
    then (reader: R) => deserializer.nullOrArrayWithNull(reader)
    else if nullable && !elemNullable
    then (reader: R) => deserializer.nullOrArray(reader)
  ...
  def ofArrayOfObjEachSuchThat(p           : JsObj => Result,
    ...
    if nullable && elemNullable
    then (reader: R) => arrOfObjParser.nullOrArrayWithNullEachSuchThat(reader,
                                                                       p(_)
                                                                       )
    else if nullable && !elemNullable
    then (reader: R) => arrOfObjParser.nullOrArrayEachSuchThat(reader,
                                                               p(_)
                                                               )
    else if !nullable && elemNullable
    then (reader: R) => arrOfObjParser.arrayWithNullEachSuchThat(reader,
                                                                 p(_)
                                                                 )
    else (reader: R) => arrOfObjParser.arrayEachSuchThat(reader,
                                                         p(_)
                                                         )
  ...
  {
    if nullable && elemNullable
    then (reader: R) => deserializer.nullOrArrayWithNullSuchThat(reader,
                                                                 p(_)
                                                                 )
    else if nullable && !elemNullable
    then (reader: R) => deserializer.nullOrArraySuchThat(reader,
                                                         p(_)
                                                         )
    else if !nullable && elemNullable
    then (reader: R) => deserializer.arrayWithNullSuchThat(reader,
                                                           p(_)
                                                           )
