...
import json.value.{JsObj, JsPath, JsValue,JsStr,JsInt,JsLong,JsDouble,JsBigDec,JsBigInt,JsBool,JsArray,JsNull}
...
object JsObjFuture
  ...
  def apply(pairs: (JsPath, Future[JsValue])*)
    ...
    def apply0(result: Future[JsObj],seq:immutable.Seq[(JsPath, Future[JsValue])]):Future[JsObj] =
      if seq.isEmpty
      then result
      else
        val head = seq.head
