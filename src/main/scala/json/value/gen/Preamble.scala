...
import json.value.{Json,JsArray, JsBigDec, JsBigInt, JsBool, JsDouble, JsInt, JsLong, JsNothing, JsNull, JsObj, JsPath, JsStr, JsValue}
...
object Preamble
{
  ...
  private[gen] def genFromPairs[T <: Json[T]](acc: Gen[T],
    ...
    then acc
                                          ...
                                          a <- pairs.head._2
                                       yield o.inserted(JsPath.empty / pairs.head._1, a )
                                  ),
                      pairs.tail
                     )
  ...
  private[gen] def concatTwo[T <: Json[T]](a: Gen[T],
    ...
    b.flatMap(it => insertedPairs(a,it.flatten.map(it => (it._1, Gen.const(it._2))): _* ))

  @scala.annotation.tailrec
  private[gen] def concatGens[T <: Json[T]](a: Gen[T], b: Gen[T], rest: Gen[T]*): Gen[T] =
    def c = concatTwo(a,b)
    if rest.isEmpty
    then c
  ...
  private[gen] def insertedPairs[T <: Json[T]](gen: Gen[T],
    ...
    def apply0(acc: Gen[T],
      ...
      val a = for
                 head <- headGen._2
                 obj <- acc
      yield obj.inserted(headGen._1, head )
      if tailGens.isEmpty
      then a
      else apply0(a, tailGens.head, tailGens.tail )
    apply0(gen,
           pairs.head,
           pairs.tail
           )
