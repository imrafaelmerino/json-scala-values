...
private[json] case class JsObjCodec(parser:Parser[JsObj]) extends Codec[JsObj]:
 override def decodeValue(in: JsonReader, default: JsObj): JsObj = parser.parse(in)
 override def encodeValue(x: JsObj, out: JsonWriter): Unit = super.encodeJsObj(x, out)
