...
final case class JsArrayOfSpecParser(private[json] val valueParser:Parser[_],min:Int,max:Int)
  ...
  override def parse(in: JsonReader) = 
    ...
    if b == '[' then 
      ...
      if array.length < min then in.decodeError("ParserSpecError.START_ARRAY_EXPECTED")
      if array.length > max then in.decodeError("ParserSpecError.START_ARRAY_EXPECTED")
      array
