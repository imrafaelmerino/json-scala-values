...
private[jsonvalues] object ErrorMessages
{
  ...
  val STRING_OF_LENGTH_LOWER_THAN_MINIMUM: (Int, Int) => String = (length: Int, minimum: Int) => s"String of length $length lower than minimum $minimum"
  val STRING_OF_LENGTH_GREATER_THAN_MAXIMUM: (Int, Int) => String = (length: Int, maximum: Int) => s"String of length $length greater than maximum $maximum"
  val STRING_DOESNT_MATCH_PATTERN: (String, String) => String = (string: String, pattern: String) => s"String $string doesn't match the pattern $pattern"
  val STRING_NOT_IN_ENUM: (String, Seq[String]) => String = (constant: String, enum: Seq[String]) => s"$constant not in $enum"
